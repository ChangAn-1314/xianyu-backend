...
# CORS中间件 - 从环境变量CORS_ORIGINS读取允许的域名（逗号分隔）
# 默认允许 http://localhost:3000（前端开发服务器）
...
async def check_version():
    """检查最新版本（已禁用外部连接，返回本地信息）"""
    return {"error": False, "message": "版本检查已禁用外部连接", "version": "local"}


@api_router.get('/version/changelog')
async def get_changelog():
    """获取更新日志（已禁用外部连接）"""
    return {"error": False, "message": "更新日志已禁用外部连接", "changelog": []}
...
# 登录速率限制：防暴力破解
_login_attempts: Dict[str, list] = {}  # {identifier: [timestamp1, timestamp2, ...]}
_LOGIN_MAX_ATTEMPTS = 5  # 最大尝试次数
_LOGIN_LOCKOUT_SECONDS = 900  # 锁定时间（15分钟）

def _check_login_rate_limit(identifier: str) -> bool:
    """检查登录速率限制，返回True表示已被锁定"""
    now = time.time()
...
def _record_login_failure(identifier: str):
    """记录登录失败"""
...
async def login(request: LoginRequest):
    ...
    # 检查登录速率限制
    login_identifier = request.username or request.email or 'unknown'
    if _check_login_rate_limit(login_identifier):
        logger.warning(f"【{login_identifier}】登录尝试次数过多，账号已锁定")
        return LoginResponse(
            success=False,
            message=f"登录尝试次数过多，请{_LOGIN_LOCKOUT_SECONDS // 60}分钟后重试"
        )
    ...
    if request.username and request.password:
        ...
        if db_manager.verify_user_password(request.username, request.password):
            ...
            if user:
                ...
                # 登录成功，清除失败记录
                _clear_login_attempts(login_identifier)
                ...
                # 检测是否仍使用默认密码
                must_change = _is_default_password(user.get('password_hash', ''))
                if must_change:
                    logger.warning(f"【{user['username']}】仍使用默认密码，强制要求修改")

                return LoginResponse(
                    success=True,
                    token=token,
                    message="登录成功，请立即修改默认密码" if must_change else "登录成功",
                    user_id=user['id'],
                    username=user['username'],
...
# 检查是否使用默认密码
@api_router.get('/check-default-password')
...
# 生成图形验证码接口
@api_router.post('/generate-captcha')
...
@api_router.get('/geetest/register', response_model=GeetestRegisterResponse)
...
@api_router.post('/geetest/validate', response_model=GeetestValidateResponse)
...
API_SECRET_KEY = os.getenv('API_SECRET_KEY', '')  # 从环境变量读取，不再硬编码
...
def verify_api_key(api_key: str) -> bool:
    ...
    try:
        ...
        if not qq_secret_key:
            logger.error("API秘钥未配置，请在系统设置或环境变量 API_SECRET_KEY 中配置")
            return False
...
@api_router.post("/xianyu/reply", response_model=ResponseModel)
...
@api_router.get("/cookies/details")
...
@api_router.put("/cookies/{cid}/login-info")
...
@api_router.post("/cookie/{cid}/account-info")
...
@api_router.get("/cookie/{cid}/details")
...
@api_router.post("/password-login")
...
@api_router.get("/face-verification/screenshot/{account_id}")
...
@api_router.delete("/face-verification/screenshot/{account_id}")
...
@api_router.post("/qr-login/generate")
...
@api_router.get("/qr-login/check/{session_id}")
...
@api_router.post("/qr-login/refresh-cookies")
...
@api_router.post("/qr-login/reset-cooldown/{cookie_id}")
...
@api_router.get("/qr-login/cooldown-status/{cookie_id}")
...
@api_router.put('/cookies/{cid}/status')
...
@api_router.get('/default-replies/{cid}')
...
@api_router.put('/default-replies/{cid}')
...
@api_router.get('/default-replies')
...
@api_router.delete('/default-replies/{cid}')
...
@api_router.post('/default-replies/{cid}/clear-records')
...
@api_router.get('/default-reply/{cid}')
...
@api_router.put('/default-reply/{cid}')
...
@api_router.delete('/default-reply/{cid}')
...
@api_router.post('/default-reply/{cid}/clear-records')
...
@api_router.get('/notification-channels/{channel_id}')
...
@api_router.get('/message-notifications')
...
@api_router.get('/message-notifications/{cid}')
...
@api_router.post('/message-notifications/{cid}')
...
@api_router.delete('/message-notifications/account/{cid}')
...
@api_router.delete('/message-notifications/{notification_id}')
...
@api_router.get('/login-info-status')
...
@api_router.put('/registration-settings')
...
@api_router.put('/login-info-settings')
...
@api_router.put("/cookies/{cid}/auto-confirm")
...
@api_router.get("/cookies/{cid}/auto-confirm")
...
@api_router.put("/cookies/{cid}/remark")
...
@api_router.get("/cookies/{cid}/remark")
...
@api_router.put("/cookies/{cid}/pause-duration")
...
@api_router.get("/cookies/{cid}/pause-duration")
...
@api_router.get("/keywords-with-item-id/{cid}")
...
@api_router.post("/keywords-with-item-id/{cid}")
...
def get_items_list(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        # 获取该账号的所有商品（使用ORM方法）
        all_items = db_manager.get_items_by_cookie(cid)
        items = []
...
@api_router.get("/keywords-export/{cid}")
...
@api_router.post("/keywords-import/{cid}")
...
@api_router.post("/keywords/{cid}/image")
...
@api_router.get("/keywords-with-type/{cid}")
...
@api_router.delete("/keywords/{cid}/{index}")
...
@api_router.get("/debug/keywords-table-info")
def debug_keywords_table_info(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "table_columns": [{"name": col['name'], "type": str(col['type']), "default": str(col.get('default', ''))} for col in columns]
        }
    except Exception as e:
...
@api_router.put("/cards/{card_id}/image")
...
@api_router.post("/delivery-rules")
...
@api_router.get("/delivery-rules/{rule_id}")
...
@api_router.put("/delivery-rules/{rule_id}")
...
@api_router.delete("/delivery-rules/{rule_id}")
...
@api_router.post("/system/reload-cache")
...
@api_router.post("/items/search_multiple")
...
@api_router.get("/items/cookie/{cookie_id}")
...
@api_router.get("/items/{cookie_id}/{item_id}")
...
@api_router.put("/items/{cookie_id}/{item_id}")
...
@api_router.delete("/items/{cookie_id}/{item_id}")
...
@api_router.get("/ai-reply-settings/{cookie_id}")
...
@api_router.put("/ai-reply-settings/{cookie_id}")
...
@api_router.get("/ai-reply-settings")
...
@api_router.post("/ai-reply-test/{cookie_id}")
...
@api_router.get("/risk-control-logs")
...
@api_router.delete("/risk-control-logs/{log_id}")
...
@api_router.post("/items/get-all-from-account")
...
@api_router.post("/items/get-by-page")
...
@api_router.put('/user-settings/{key}')
...
@api_router.get('/user-settings/{key}')
...
@api_router.get('/admin/risk-control-logs')
...
@api_router.get('/admin/log-files')
...
@api_router.get("/itemReplays/cookie/{cookie_id}")
def get_items_by_cookie(cookie_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    """获取指定Cookie的商品信息"""
    try:
        # 检查cookie是否属于当前用户
...
@api_router.put("/item-reply/{cookie_id}/{item_id}")
...
@api_router.delete("/item-reply/{cookie_id}/{item_id}")
...
@api_router.delete("/item-reply/batch")
...
@api_router.get("/item-reply/{cookie_id}/{item_id}")
...
@api_router.post('/admin/backup/download')
def download_database_backup(request: Dict[str, Any], admin_user: Dict[str, Any] = Depends(require_admin)):
    """下载数据库备份文件（管理员专用，需二次密码验证）"""
    import os
    ...
    try:
        # 安全检查：使用默认密码的管理员禁止下载
        from db_manager import db_manager
        admin_info = db_manager.get_user_by_username(admin_user.get('username', ''))
        if admin_info and _is_default_password(admin_info.get('password_hash', '')):
            raise HTTPException(status_code=403, detail="使用默认密码时禁止下载数据库，请先修改管理员密码")

        # 二次密码验证
        confirm_password = request.get('confirm_password', '')
        if not confirm_password:
            raise HTTPException(status_code=400, detail="请输入管理员密码进行二次验证")
        if not db_manager.verify_user_password(admin_user.get('username', ''), confirm_password):
            log_with_user('warning', "数据库下载二次密码验证失败", admin_user)
            raise HTTPException(status_code=403, detail="密码验证失败")

        log_with_user('info', "请求下载数据库备份（已通过二次验证）", admin_user)
...
@api_router.post('/admin/backup/upload')
...
@api_router.post('/admin/reload-cache')
...
@api_router.get('/admin/data/{table_name}')
...
@api_router.delete('/admin/data/{table_name}/{record_id}')
...
@api_router.delete('/admin/data/{table_name}')
...
# 商品多规格管理API
@api_router.put("/items/{cookie_id}/{item_id}/multi-spec")
...
# 商品多数量发货管理API
@api_router.put("/items/{cookie_id}/{item_id}/multi-quantity-delivery")
...
# 注册API路由器到app（必须在catch-all路由之前）
app.include_router(api_router)
logger.info("已注册api_router，所有业务API统一使用/api/前缀")
