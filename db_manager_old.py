...
class DBManager:
    ...
    # SQL注入防护：允许的表名白名单
    ALLOWED_TABLES = {
        'cookies', 'cookie_status', 'keywords', 'item_info',
        'ai_reply_settings', 'ai_conversations', 'ai_item_cache', 'users',
        'system_settings', 'notification_channels', 'old_notification_channels',
        'delivery_cards', 'delivery_rules', 'delivery_logs', 'cards',
        'order_details_cache', 'item_replies', 'item_replay', 'sqlite_sequence',
        'verification_codes', 'default_replies', 'default_reply_records',
        'message_notifications', 'orders', 'user_settings',
        'email_verifications', 'captcha_codes', 'risk_control_logs',
    }

    @staticmethod
    def _validate_table_name(table_name: str) -> str:
        """验证表名是否在白名单中，防止SQL注入"""
        if table_name not in DBManager.ALLOWED_TABLES:
    ...
    def init_db(self):
        ...
        try:
            ...
            ('qq_reply_secret_key', '', 'QQ回复消息API秘钥（请在系统设置中配置）')
            ''')
    ...
    def update_admin_user_id(self, cursor):
        ...
        try:
            ...
            if not admin_exists:
                # 首次创建admin用户，设置默认密码
                default_password_hash = bcrypt_hash.hash("admin123")
                cursor.execute('''
                INSERT INTO users (username, email, password_hash) VALUES
                ('admin', 'admin@localhost', ?)
                ''', (default_password_hash,))
                logger.info("创建默认admin用户（首次登录后请立即修改密码）")
